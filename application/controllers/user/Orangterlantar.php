...
class Orangterlantar extends MY_user_controller
{
  ...
  public function datatables()
  {
    try {
      ...
      if ($method == 'POST') {
        ...
        $params['like'] = $like;
        $this->datatable_data($this->Terlantar, $params);
      } else {
  ...
  public function add()
  {
    ...
    $data['_kebutuhan_diperlukans'] = $this->loopingSelect('kebutuhan_diperlukan', $this->KebutuhanDiperlukan->get_all_kebutuhan_diperlukan(), ['id' => "0", 'nama' => '-- Isi Kebutuhan sendiri --']);
    ...
    if (isset($_POST['kirim'])) {
      ...
      $this->form_validation->set_rules('nik', 'NIK Orang Terlantar', 'min_length[16]|max_length[16]');
      $this->form_validation->set_rules('no_kk', 'NO KK Orang Terlantar', 'min_length[16]|max_length[16]');
      ...
      //   $this->form_validation->set_rules('no_telp','No Telp Orang Terlantar','required');
      // }
      ...
      $this->form_validation->set_rules('kondisi_tempat_tinggal', 'Kondisi Tempat Tinggal', "callback_validateSelectKondisiTempatTinggal[{$_POST['tempat_tinggal']}]");
      ...
      $this->form_validation->set_rules('kebutuhan_diperlukan', 'Kebutuhan Yang Diperlukan', 'callback_validateSelect[true]');
      ...
      $this->form_validation->set_message('validateSelectKondisiTempatTinggal', 'harus pilih {field}');
      if ($this->form_validation->run()) {
        try {
          ...
          $params['terlantar_tempat_lahir'] =  htmlspecialchars(strtoupper($this->input->post('tempat_lahir')));
          $params['terlantar_tanggal_lahir'] =  htmlspecialchars(date('Y-m-d', strtotime($this->input->post('tanggal_lahir'))));
          $params['terlantar_jenis_kelamin'] =  htmlspecialchars(strtoupper($this->input->post('jenis_kelamin')));
          $params['agama_id'] =  $_POST['agama'];
          ...
          // }
          if ($_POST['kebutuhan_diperlukan'] == "0") {
            $params['kebutuhan_diperlukan_lainnya'] = htmlspecialchars($this->input->post('kebutuhan_diperlukan_lainnya'));
            unset($params['kebutuhan_diperlukan_id']);
          }
          if ($_POST['tempat_tinggal'] == "3") {
            unset($params['kondisi_tempat_tinggal_id']);
          }
          if ($this->Terlantar->add_terlantar($params) >= 1) {
            $this->session->set_flashdata('success', "Berhasil mengirimkan data orang terlantar");
            redirect('user/dashboard');
          } else {
            throw new Exception("Gagal mengirimkan data orang terlantar");
          }
        } catch (Exception $th) {
  ...
  public function edit($id)
  {
    try {
      ...
      if (!$terlantar) {
        ...
        $this->load->model('kondisi_ot_model', 'KondisiOt');
        $this->load->model('kebutuhan_diperlukan_model', 'KebutuhanDiperlukan');
        $data['_view'] = 'user/orang_terlantar/edit';
        $data['_title'] = 'Edit Orang Terlantar ' . $terlantar['terlantar_nama'];
        $terlantar['kebutuhan_diperlukan_id'] = $terlantar['kebutuhan_diperlukan_id'] ?? "0";
        ...
        if ($terlantar['proses_id'] != null || $terlantar['verif_id'] != null || $terlantar['tolak_id'] != null) {
          $this->session->set_flashdata('error', 'Data sudah diproses oleh admin, tidak bisa diedit');
          redirect('user/dashboard');
        }
        if (isset($_POST['kirim'])) {
          $this->load->library('form_validation');
          ...
          if ($this->form_validation->run()) {
            try {
              ...
              if ($_FILES['foto']['type'] != null) {
                $params['foto'] = $this->_do_upload('foto', $terlantar['no_urut']);
              }
              $params['user_daftar'] = $this->session->user_id;
              // if(isset($_POST['have_identitas'])){
              $params['terlantar_nik'] =  htmlspecialchars($this->input->post('nik'));
              ...
              if ($_POST['kebutuhan_diperlukan'] == "0") {
                $params['kebutuhan_diperlukan_lainnya'] = htmlspecialchars($this->input->post('kebutuhan_diperlukan_lainnya'));
                $params['kebutuhan_diperlukan_id'] = null;
              }
              if ($this->Terlantar->update_terlantar($terlantar['terlantar_id'], $params) >= 1) {
                $this->session->set_flashdata('success', "Berhasil mengupdate data orang terlantar");
                redirect('user/dashboard');
              } else {
                throw new Exception("Gagal mengubah data orang terlantar");
              }
            } catch (Exception $th) {
              $this->session->set_flashdata('error', $th->getMessage());
              $this->load->view('layout', $data);
  ...
  function validateSelectKondisiTempatTinggal($str, $tempat_tinggal)
  {
    if ($tempat_tinggal === "3") {
      return true;
    } else {
  ...
  {
    $data['_view'] = 'user/orang_terlantar/list';
    $data['_title'] = 'List Orang Terlantar Yang Terdaftar';
    $data['_fields'] = $this->Terlantar->count_field('terlantar_kecamatan');
    $this->load->view('layout', $data);
  }
}
