...
<script>
    ...
    function batalTolak(id) {
        var cek = confirm('Yakin ingin membatalkan penolakan data ini?');
        if (cek) {
            $('.batalTolak').attr('action', "<?= base_url('admin/orangterlantar/tolak/') ?>" + id)
            $('.batalTolak').submit();
        }
    }
            ...
            "columns": [{
                ...
                },
                {
                    "render": function(data, type, row, meta) { // Tampilkan kolom aksi
                        ...
                        html += '<form method="POST" class="batalTolak float-left"><input type="hidden" name="batal"><button type="button" class="btn btn-sm" onClick="batalTolak(' + row.terlantar_id + ')"><div class="badge badge-success"><i class="fa fa-fw fa-times"></i> Batal Tolak</div></form>'
