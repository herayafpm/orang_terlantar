...
<script>
    ...
    function batalVerif(id) {
        var cek = confirm('Yakin ingin membatalkan verifikasi data ini?');
        if (cek) {
            $('.batalVerif').attr('action', "<?= base_url('admin/orangterlantar/verif/') ?>" + id)
            $('.batalVerif').submit();
        }
    }
            ...
            "columns": [{
                ...
                },
                {
                    "render": function(data, type, row, meta) { // Tampilkan kolom aksi
                        ...
                        html += '<form method="POST" class="batalVerif float-left"><input type="hidden" name="batal"><button type="button" class="btn btn-sm" onClick="batalVerif(' + row.terlantar_id + ')"><div class="badge badge-success"><i class="fa fa-fw fa-times"></i> Batal Verif</div></form>'
